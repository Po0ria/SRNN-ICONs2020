...
parser = argparse.ArgumentParser(description='PyTorch SRNN Training')
parser.add_argument('--detail', default='', type=str, help='You want it in model directory')
parser.add_argument('--epochs', default='50', type=str, help='number of epochs')
parser.add_argument('--drop_rate', default='0.2', type=str, help='dropout rate')
global args
...
train_X = np.load('./SHD/data/10ms/trainX_10ms.npy')
train_y = np.load('./SHD/data/10ms/trainY_10ms.npy').astype(np.float)

test_X = np.load('./SHD/data/10ms/testX_10ms.npy')
...
class RNN_custom(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.drop = nn.Dropout(p=drop_rate, inplace=False)
    ...
    def forward(self, input):
        ...
        dropped_spike_layer1 = Variable(torch.zeros(batch_size, self.hidden_size[0]).cuda(), requires_grad=False)
        dropped_spike_layer2 = Variable(torch.zeros(batch_size, self.hidden_size[1]).cuda(), requires_grad=False)
        mask_i = Variable(torch.ones(batch_size, input_dim).cuda(), requires_grad=False) 
        mask_1 = Variable(torch.ones(batch_size, self.hidden_size[0]).cuda(), requires_grad=False) 
        mask_2 = Variable(torch.ones(batch_size, self.hidden_size[1]).cuda(), requires_grad=False) 
        mask_i = self.drop(mask_i)
        mask_1 = self.drop(mask_1)
        mask_2 = self.drop(mask_2)
        



        self.b_h1 = self.b_h2 = self.b_o = b_j0
        #print("this is seq_num" + str(seq_num) + '\n')
        ...
        hidden_mem_ = []
        h2o_mem_ = []
        #print("Density of input :  " + str(float(np.count_nonzero(input.cpu().detach().numpy()))/torch.numel(input)) + " dimension " + str(input.shape))
        h_density = 0
        for i in range(seq_num):
            #import pdb;pdb.set_trace()
            input_x = input[:, i, :]
            #****Dropout**
            dropped_input_x = torch.mul(input_x,mask_i)            
            h_input = self.i_2_h1(dropped_input_x.float()) + self.h1_2_h1(dropped_spike_layer1)
           
            mem_layer1, spike_layer1, theta_h1, self.b_h1 = mem_update_adp(h_input, mem_layer1, dropped_spike_layer1,
                                                                         self.tau_adp_h1, self.b_h1,self.tau_m_h1)
            #****Dropout**
            dropped_spike_layer1 = torch.mul(spike_layer1, mask_1)

            h2_input = self.h1_2_h2(spike_layer1) + self.h2_2_h2(spike_layer2)

            mem_layer2, spike_layer2, theta_h2, self.b_h2 = mem_update_adp(h2_input, mem_layer2, dropped_spike_layer2,
                                                                         self.tau_adp_h2, self.b_h2, self.tau_m_h2)
            #****Dropout**
            dropped_spike_layer2 = torch.mul(spike_layer2,mask_2) 

            mem_output = output_Neuron(self.h2o(dropped_spike_layer2), mem_output, self.tau_m_o)
            if i > 0:
                output= output + F.softmax(mem_output, dim=1)

            hidden_spike_.append(dropped_spike_layer1.data.cpu().numpy())
            hidden_spike_2.append(dropped_spike_layer2.data.cpu().numpy())
...
def train(model, num_epochs=150):
    ...
    for epoch in range(num_epochs):
        ...
        if ts_acc > best_accuracy and accuracy > 80:
            torch.save(model, './model/dropout/' + detail +'_'+ str(drop_rate) + '_'+ str(ts_acc) + '.pth')
...
def test(model, dataloader=test_loader):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module,nn.Linear):
            writer.add_histogram("weights", module.weight, step)    
...
model = torch.load('./model/dropout/50epoch_0.5_82.8125.pth')
#import pdb;pdb.set_trace()
accuracy = test(model)
model = torch.load('./model/dropout/50epoch_0.2_89.0625.pth')
accuracy = test(model)
