...
class RNN_custom(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(seq_num):
            ...
            #print("Density of h_input :  " + str(float(np.count_nonzero(self.i_2_h1(input_x.float()).cpu().detach().numpy()))/torch.numel(self.i_2_h1(input_x.float()))) + " dimension " + str(self.i_2_h1(input_x.float()).shape))
            #print("Density of spike_layer1 before:  " + str(float(np.count_nonzero(spike_layer1.cpu().detach().numpy()))/torch.numel(spike_layer1)) + " dimension " + str(spike_layer1.shape))
           # print("Density of h1_2_h1 :  " + str(float(np.count_nonzero(self.h1_2_h1(spike_layer1).cpu().detach().numpy()))/torch.numel(self.h1_2_h1(spike_layer1))) + " dimension " + str(self.h1_2_h1(spike_layer1).shape))
            ...
            #print("Density of spike_layer2:  " + str(float(np.count_nonzero(spike_layer2.cpu().detach().numpy()))/torch.numel(spike_layer2)) + " dimension " + str(spike_layer2.shape))

            h2_input = self.h1_2_h2(spike_layer1) + self.h2_2_h2(spike_layer2)
            #print("Density of h2 :  " + str(float(np.count_nonzero(h2_input.cpu().detach().numpy()))/torch.numel(h2_input)) + " dimension " + str(h2_input.shape))
            ...
            if i > 0:
               ...
               # print("Density of output :  " + str(float(np.count_nonzero(output.cpu().detach().numpy()))/torch.numel(output)) + " dimension " + str(output.shape))

            hidden_spike_.append(spike_layer1.data.cpu().numpy())
            hidden_spike_2.append(spike_layer2.data.cpu().numpy())
            hidden_mem_.append(mem_layer1.data.cpu().numpy())
            h2o_mem_.append(mem_output.data.cpu().numpy())
        # print("Density of hidden :  " + str(float(np.count_nonzero(torch.FloatTensor(hidden_spike_)))/torch.numel(torch.FloatTensor(hidden_spike_))))
        # print("Density of hidden2 :  " + str(float(np.count_nonzero(torch.FloatTensor(hidden_spike_2)))/torch.numel(torch.FloatTensor(hidden_spike_2))))
...
def train(model, num_epochs=150):
    ...
    for epoch in range(num_epochs):
        ...
        if ts_acc > best_accuracy and accuracy > 80:
            torch.save(model, './model/original_4ms/model_' + str(ts_acc) + '-readout-2layer.pth')
...
#model = torch.load('./model/original/model_92.96875-readout-2layer.pth')
accuracy = test(model)
