...
parser.add_argument('--prune_rate', default='0.7', type=str, help='Pruning Rate')
global args
...
def train(model, num_epochs=150):
    ...
    for epoch in range(num_epochs):
        ...
        if ts_acc > best_accuracy and accuracy > 80:
            torch.save(model, './model/prune/' + detail +'/'+ str(prune_rate) + '_'+ str(ts_acc) + '.pth')
...
###############################
model = torch.load('./model/original_10ms/model_92.96875-readout-2layer.pth')
from torch.nn.utils import prune
#import pdb;pdb.set_trace()
for name, module in model.named_modules():
    if isinstance(module, nn.Linear):
        prune.l1_unstructured(module, 'weight', prune_rate)
        prune.remove(module, 'weight') 
extra_epoch = 20
acc = train(model, extra_epoch)
accuracy = test(model)
